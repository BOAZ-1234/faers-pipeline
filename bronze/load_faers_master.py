#FAERS 원본 데이터를 완벽하게 브론즈 계층에 쌓는 메인 마스터 적재 스크립트입니다. data/ 폴더 안의 모든 ZIP 파일을 찾아 연속으로 압축 해제 없이 다이렉트 스트리밍합니다. 환자(DEMO), 약물(DRUG), 부작용(REAC) 데이터를 분리하여 stage_a_raw 네임스페이스 안의 각각의 테이블로 3만 건씩 분할 적재합니다. (환자는 덮어쓰기, 약물/부작용은 이어붙이기 방식을 적용하는 매우 정교한 코드입니다.)
...
# run() 실행 전(예: 테스트에서 import만 할 때)에는 비어있다.
# 테스트에서는 이 모듈을 import한 뒤 이 값을 로컬 Spark 세션으로 바꿔치기해서 쓴다.
spark = None
...
def classify_line(raw_line, header, file_name, target, file_type, line_no):
    ...
    컬럼 개수가 header와 다르면 버리지 않고 격리 레코드로 만든다 (설계 문서 4-1).
    """
